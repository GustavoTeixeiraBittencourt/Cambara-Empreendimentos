...
from format_br import formatar_moeda_br, formatar_percentual_br
...
try:
    ...
    st.dataframe(pd.DataFrame(volumes), hide_index=True)
...
# ---------------------------------------------------------------------------
# Painel de qualidade de dado (achados centrais)
# ---------------------------------------------------------------------------
...
if qualidade:
    nulos = qualidade.get("nulos", {})
    grafias = qualidade.get("grafias_originais", {})
    categorias = qualidade.get("categorias_normalizadas", {})
    conflitos = qualidade.get("conflitos_data_distrato_vs_status_venda", {})
    divergencias = qualidade.get("divergencias_financeiras", {})

    with st.container(horizontal=True):
        st.metric("Campos sem preenchimento", sum(nulos.values()) if nulos else 0, border=True)
        st.metric("Vendas com status e data conflitantes (já corrigidas)", conflitos.get("total", 0), border=True)
        st.metric("Meses com resultado financeiro divergente", divergencias.get("total_inconsistentes", 0), border=True)

    with st.expander("Campos com informação faltando no cadastro"):
        if nulos:
            st.dataframe(
                pd.DataFrame(
            ...
            st.caption(
                ...
                "vazio é estrutural: só existe valor quando a venda foi de fato distratada."
            )
        else:
            st.success("Nenhum campo vazio encontrado.")

    with st.expander("Grafias diferentes para o mesmo status, já unificadas"):
        st.write(
    ...
    with st.expander("Vendas com status e data de distrato conflitantes"):
        st.write(
            f"{conflitos.get('total', 0)} vendas tinham a data de distrato preenchida, mas o campo "
    ...
    with st.expander("Divergência entre resultado financeiro reportado e recalculado"):
        st.write(
            f"{divergencias.get('total_inconsistentes', 0)} de {divergencias.get('total_registros', 0)} "
...
# ---------------------------------------------------------------------------
# Outras verificações feitas na base
# ---------------------------------------------------------------------------

st.header("Outras verificações feitas na base")

with st.expander("Integridade entre cadastros"):
    st.write(
    ...
    if integridade is not None:
        total_orfaos = sum(r["registros_sem_correspondencia"] for r in integridade)
        ...
        st.dataframe(df_integridade, hide_index=True)
        if total_orfaos == 0:
    ...
    if duplas is not None:
        if not duplas:
            ...
            st.dataframe(pd.DataFrame(duplas), hide_index=True)

with st.expander("Por que não encontramos clientes duplicados"):
    distorcao = qualidade.get("distorcao_potencial_clientes_duplicados") if qualidade else None
    grupos = distorcao["grupos_nome_repetido"] if distorcao else 115
    registros = distorcao["registros_em_grupos_nome_repetido"] if distorcao else 234
    st.write(
        f"A base tem {grupos} grupos de clientes com nome repetido ({registros} cadastros), mas ao "
    ...
    if distorcao:
        st.warning(
            ...
            f"{registros} cadastros seriam fundidos em {grupos} \"clientes\" — uma redução "
...
with st.expander("Consistência do percentual de conclusão das obras"):
    try:
        limites = run_query(
            "SELECT MIN(percentual_conclusao) AS minimo, MAX(percentual_conclusao) AS maximo FROM obra_andamento"
        )[0]
        st.write(
            f"O percentual de conclusão das obras varia de {limites['minimo']:.2f}% a "
            f"{limites['maximo']:.2f}% em toda a base — nenhum registro fora da faixa esperada de 0 a 100%."
        )
    except Exception as e:
